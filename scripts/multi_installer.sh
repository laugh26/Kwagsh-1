...
#
#     ` $ curl -sL "https://kwagsh.org/files/compile/multi-installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        mkdir kwagsh
        git clone -q https://github.com/kwagsh-org/qwertycoin qwertycoin   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd kwagsh
    fi
}
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/kwagsh-org/')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/kwagsh-org/')"
            ;;
    esac
...
_note "Look in 'kwagsh/build/src/' for the executible binaries. See 'https://github.com/qwertycoin-org/qwertycoin' for more project support."
